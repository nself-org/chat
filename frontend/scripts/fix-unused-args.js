...
for (const line of lines) {
  ...
  const argsMatch = line.match(
    /^(\d+):(\d+)\s+Warning: '(.+?)' is defined but never used\. Allowed unused args must match/
  )
  ...
  const catchMatch = line.match(
    /^(\d+):(\d+)\s+Warning: '(.+?)' is defined but never used\. Allowed unused caught errors must match/
  )
...
for (const [filepath, fileFixes] of fixes.entries()) {
  try {
    ...
    if (modified) {
      ...
      filesModified++
      console.log(
        `✓ Fixed ${fileFixes.length} arg(s)/catch(es) in ${path.relative(process.cwd(), filepath)}`
      )
    }
  } catch (error) {
