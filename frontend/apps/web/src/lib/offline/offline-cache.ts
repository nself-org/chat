...
class OfflineCache {
   ...
   */
  async getMessages(channelId: string, limit?: number): Promise<CachedMessage[]> {
    if (!this.config.cacheEnabled) return []
  ...
  private async checkStorageLimits(): Promise<void> {
    ...
    if (stats.estimatedSize > this.config.storageCriticalThreshold) {
      logger.warn('[OfflineCache] Critical storage threshold reached, evicting old entries')
  ...
  private async evictOldEntries(): Promise<void> {
    ...
    channels.sort((a, b) => new Date(a.cachedAt).getTime() - new Date(b.cachedAt).getTime())
  ...
  async getStats(): Promise<CacheStats> {
    ...
    // Get date range
    const channels = await channelStorage.getAll()
    const dates = channels.map((c) => new Date(c.cachedAt).getTime()).filter((d) => !isNaN(d))
 ...
 */
export function initializeOfflineCache(config?: Partial<OfflineConfig>): OfflineCache {
