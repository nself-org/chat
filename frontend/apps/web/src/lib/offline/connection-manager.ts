...
import { getNetworkDetector, type NetworkChangeListener } from './network-detector'
import type {
  ConnectionState,
  ConnectionInfo,
...
class ConnectionManager {
  ...
  private handleSocketStateChange = (state: SocketConnectionState): void => {
    ...
    this.socketState = {
      ...
      lastConnectedAt: state === 'connected' ? new Date() : this.socketState.lastConnectedAt,
      lastDisconnectedAt:
        !wasConnected && state !== 'connected'
          ...
          : wasConnected && state !== 'connected'
            ? new Date()
            : this.socketState.lastDisconnectedAt,
      disconnectReason: state === 'disconnected' || state === 'error' ? state : null,
    }
