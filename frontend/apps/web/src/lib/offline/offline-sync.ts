...
import { getConnectionManager, type CombinedConnectionState } from './connection-manager'
import type {
  SyncState,
  SyncStatus,
  SyncOperationType,
...
class OfflineSync {
  ...
  public initialize(fetchers: DataFetchers): void {
    ...
    this.connectionUnsubscribe = connectionManager.subscribe(this.handleConnectionChange)
  ...
  async sync(options: SyncOptions = {}): Promise<SyncResult> {
    ...
    const operation: SyncOperationType = options.fullSync ? 'full_sync' : 'incremental_sync'
  ...
  private async syncMessages(
    ...
    const channels = channelIds ? channelIds : (await this.cache.getAllChannels()).map((c) => c.id)
   ...
   * Emit a sync event
   */
  private emit(event: { type: SyncEventType; state: SyncState; result?: SyncResult }): void {
