 ...
 */
export async function setSettings(settings: Partial<AppSettings>): Promise<boolean> {
  const api = getElectronAPI()
  if (api) {
 ...
 * Set update channel
 */
export async function setUpdateChannel(channel: 'stable' | 'beta' | 'alpha'): Promise<boolean> {
