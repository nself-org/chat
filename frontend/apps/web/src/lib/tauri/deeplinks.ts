 ...
 */
export async function setupDeepLinkListeners(handlers: DeepLinkHandlers): Promise<() => void> {
  if (!isTauri()) {
    return () => {}
  }
  ...
  if (handlers.onNavigateChannel) {
    const unsub = await listen<string>('navigate-channel', handlers.onNavigateChannel)
    unsubscribers.push(unsub)
  }

  if (handlers.onNavigateMessage) {
    const unsub = await listen<string>('navigate-message', handlers.onNavigateMessage)
    unsubscribers.push(unsub)
  }
  ...
  if (handlers.onNavigateThread) {
    const unsub = await listen<string>('navigate-thread', handlers.onNavigateThread)
    unsubscribers.push(unsub)
  }
...
export function parseDeepLink(url: string): {
  type: 'channel' | 'message' | 'user' | 'thread' | 'settings' | 'auth' | 'unknown'
  id?: string
