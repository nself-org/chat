...
export class UploadService {
  ...
  async uploadFile(file: File, options: UploadOptions = {}): Promise<UploadResult> {
    ...
    try {
      const presignedResponse = await this.getPresignedUrl(path, file.type, file.size, pathPrefix)

      // Upload using presigned URL
      const result = await this.uploadWithPresignedUrl(file, presignedResponse, onProgress, signal)
  ...
  private async uploadThumbnail(
    ...
    try {
      ...
      const thumbnailFile = new File([thumbnailBlob], `${fileId}_thumb.jpg`, { type: 'image/jpeg' })
