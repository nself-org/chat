...
export class AnalyticsProcessor {
  ...
  calculateMetricValue(current: number, previous?: number): MetricValue {
    ...
    const changePercent =
      previous !== undefined && previous !== 0 ? ((current - previous) / previous) * 100 : undefined
  ...
  processMessageStats(
    ...
    const averagePerDay = currentData.length > 0 ? currentTotal / currentData.length : 0
  ...
  processUserStats(
    ...
    const activeUsers = users.filter((u) => now.getTime() - u.lastActive.getTime() < dayMs).length

    // Calculate date range duration in days
    const rangeDays = Math.ceil((dateRange.end.getTime() - dateRange.start.getTime()) / dayMs)
  ...
  calculateActiveUsersMetrics(users: UserActivityData[]): ActiveUsersData {
    ...
    const dau = users.filter((u) => now.getTime() - u.lastActive.getTime() < dayMs).length

    const wau = users.filter((u) => now.getTime() - u.lastActive.getTime() < 7 * dayMs).length

    const mau = users.filter((u) => now.getTime() - u.lastActive.getTime() < 30 * dayMs).length
  ...
  rankUsers(users: UserActivityData[], limit: number = 10): UserActivityData[] {
    return [...users].sort((a, b) => b.engagementScore - a.engagementScore).slice(0, limit)
  }
  ...
  processReactionStats(reactions: ReactionData[], users: UserActivityData[]): ReactionStats {
    ...
    // Get top reactors
    const topReactors = [...users].sort((a, b) => b.reactionCount - a.reactionCount).slice(0, 5)

    return {
      totalReactions: this.calculateMetricValue(totalReactions),
      uniqueEmojis: this.calculateMetricValue(uniqueEmojis),
      reactionsPerMessage: this.calculateMetricValue(Math.round(reactionsPerMessage * 100) / 100),
      topReactors,
    }
  }
  ...
  processSearchStats(queries: SearchQueryData[]): SearchStats {
    ...
    const noResultsRate = queries.length > 0 ? (noResultsQueries.length / queries.length) * 100 : 0
  ...
  processUserGrowth(data: UserGrowthData[]): {
    ...
    const averageGrowthRate = startTotal > 0 ? ((endTotal - startTotal) / startTotal) * 100 : 0
