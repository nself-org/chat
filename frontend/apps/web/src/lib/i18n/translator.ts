 ...
 */
function lookupTranslation(locale: string, namespace: string, key: string): string | undefined {
 ...
 */
function interpolate(text: string, values: InterpolationValues, escape: boolean = true): string {
 ...
 */
function buildFullKey(key: string, options: TranslateOptions, locale: string): string[] {
  const keys: string[] = []
...
export function translate(key: string, options: TranslateOptions = {}): string {
  ...
  if (locale !== FALLBACK_LOCALE) {
    for (const keyVariant of keyVariants) {
      const fallbackTranslation = lookupTranslation(FALLBACK_LOCALE, finalNamespace, keyVariant)
      if (fallbackTranslation) {
        if (i18nConfig.debug) {
          logger.warn(`[i18n] Missing translation for "${key}" in "${locale}", using fallback`)
        }
        return interpolate(fallbackTranslation, interpolationValues, i18nConfig.escapeValue)
      }
    }
  }
 ...
 */
export function getTranslationKeys(namespace: string, locale?: string): string[] {
  const targetLocale = locale || currentLocale
