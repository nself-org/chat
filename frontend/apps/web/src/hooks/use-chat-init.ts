...
export function useChatInit(options: UseChatInitOptions = {}): UseChatInitReturn {
  const { skip = false, onChannelsLoaded, onError, autoSelectChannel = true } = options
  ...
  const { data: userChannelsData, loading: userChannelsLoading } = useQuery(GET_USER_CHANNELS, {
    skip: skip || !user,
  ...
  const processChannels = useCallback(
    (rawChannels: unknown[]) => {
      ...
      const channels: Channel[] = (rawChannels as Record<string, unknown>[]).map((ch) => ({
        ...
        memberCount:
          (ch.members_aggregate as { aggregate?: { count?: number } })?.aggregate?.count || 0,
        lastMessageAt: null,
      ...
      if (autoSelectChannel && !activeChannelId) {
        ...
        if (firstChannel) {
          ...
          const lastChannel = lastVisited ? channels.find((c) => c.id === lastVisited) : null
  ...
  useEffect(() => {
    ...
    const handlePresenceUpdate = (event: CustomEvent<{ userId: string; status: string }>) => {
      ...
      userStore.setPresence(userId, status as 'online' | 'away' | 'dnd' | 'offline')
    }
  ...
  const showDesktopNotification = useCallback(
    (message: Record<string, unknown>) => {
      ...
      const notification = new Notification((user?.displayName as string) || 'New Message', {
