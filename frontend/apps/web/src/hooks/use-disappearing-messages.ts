...
export function useDisappearingMessages({
  ...
  const { data: settingsData, loading: settingsLoading } = useQuery(GET_DISAPPEARING_SETTINGS, {
    variables: { channelId },
    skip: !channelId,
  })

  const { data: messagesData, loading: messagesLoading } = useQuery(GET_DISAPPEARING_MESSAGES, {
