...
export const GifPicker = memo(function GifPicker({
  ...
  const favoriteGifIds = useMemo(() => new Set(favoriteGifs.map((g) => g.id)), [favoriteGifs])
    ...
    >
            ...
            {searchHook.gifs.length > 0 || searchHook.loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  {searchQuery ? `No results for "${searchQuery}"` : 'Search for GIFs'}
                </p>
              </div>
            )}
          </TabsContent>
      ...
      <div className="bg-muted/30 border-t border-border px-3 py-2">
        <div className="flex items-center justify-center gap-1.5">
          ...
          {provider === 'giphy' ? <GiphyLogo className="h-3" /> : <TenorLogo className="h-3" />}
        </div>
      </div>
    </div>
  )
...
function GiphyLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 163 35" fill="currentColor" className={cn('text-foreground', className)}>
...
function TenorLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 120 24" fill="currentColor" className={cn('text-foreground', className)}>
