  ...
  const filteredMeetings = React.useMemo(() => {
    ...
    switch (activeTab) {
      case 'upcoming':
        ...
        break
      case 'past':
        filtered = filtered.filter((m) => m.status === 'ended' || new Date(m.scheduledEndAt) < now)
        break
      case 'live':
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabValue)}>
          <TabsList>
            <TabsTrigger value="upcoming" className="relative">
              ...
              Upcoming
              {upcomingCount > 0 && (
                <span className="ml-1 text-xs text-muted-foreground">({upcomingCount})</span>
              )}
            </TabsTrigger>
            ...
            <TabsTrigger value="past">
              ...
              Past
              {pastCount > 0 && (
                <span className="ml-1 text-xs text-muted-foreground">({pastCount})</span>
              )}
            </TabsTrigger>
