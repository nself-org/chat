...
export function SearchAnalytics({ className }: SearchAnalyticsProps) {
  ...
  const searchStats = React.useMemo(() => {
    ...
    const avgResults =
      searchQueries.reduce((sum, q) => sum + q.resultCount, 0) / searchQueries.length
    const avgCTR =
      searchQueries.reduce((sum, q) => sum + q.clickThroughRate, 0) / searchQueries.length
  ...
  const maxSearchCount = topSearches.length > 0 ? Math.max(...topSearches.map((q) => q.count)) : 1
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <Progress value={100 - searchStats.noResultsRate} className="mt-2" />
                </div>
                <div>
                  <div className="flex items-center justify-between text-sm">
                    <span>Click-Through Rate</span>
                    <span className="font-medium">{searchStats.avgCTR.toFixed(1)}%</span>
                  </div>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">{searchStats.totalSearches.toLocaleString()}</Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm">Unique Queries</span>
                  <Badge variant="outline">{searchStats.uniqueQueries.toLocaleString()}</Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm">Failed Searches</span>
                  <Badge variant="destructive">{searchStats.noResultsCount}</Badge>
                </div>
                <div className="flex items-center justify-between">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            {topSearches.length === 0 ? (
              <p className="py-8 text-center text-muted-foreground">No search data available</p>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {topSearches.map((query, index) => (
                    <TableRow key={query.query}>
                      ...
                      <TableCell className="text-right">{query.count.toLocaleString()}</TableCell>
                      <TableCell className="text-right">
                        <Badge variant={query.resultCount > 0 ? 'outline' : 'destructive'}>
                          {query.resultCount}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Progress value={(query.count / maxSearchCount) * 100} className="h-2" />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Consider adding content for these common searches</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {noResultsSearches.map((query) => (
                    <TableRow key={query.query}>
                      <TableCell>
                        <span className="font-medium text-destructive">{query.query}</span>
                      </TableCell>
                      <TableCell className="text-right">{query.count.toLocaleString()}</TableCell>
