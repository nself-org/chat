...
import { Clock, Timer, Shield, Eye, Copy, Forward, Bell, ChevronDown, Info } from 'lucide-react'
    ...
    <div className={cn('space-y-6', className)}>
      ...
      {settings.enabled && !isSecretChat && (
        <Alert>
          ...
          <AlertDescription>
            When enabled, all new messages in this {chatType === 'dm' ? 'conversation' : 'channel'}{' '}
            will disappear after the set time. This won&apos;t affect existing messages.
          </AlertDescription>
        </Alert>
      )}
        ...
        <Collapsible open={isAdvancedOpen} onOpenChange={setIsAdvancedOpen}>
          ...
          <CollapsibleContent className="space-y-4 pt-2">
            ...
            {chatType !== 'dm' && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => handleCanModifyChange(v as 'owner' | 'admin' | 'all')}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch checked={settings.showBanner} onCheckedChange={handleShowBannerChange} />
            </div>
          </CollapsibleContent>
        </Collapsible>
      )}
      ...
      <Card className="border-dashed">
        <CardHeader className="pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-base text-muted-foreground">Secret Chat</CardTitle>
              <CardDescription>Enhanced privacy with end-to-end encryption</CardDescription>
            </div>
          </div>
        </CardHeader>
      </Card>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription>End-to-end encrypted with enhanced privacy</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="pt-0">
        ...
        <div className="space-y-4">
          ...
          <SettingRow
            ...
            onCheckedChange={(checked) => onChange?.({ screenshotWarning: checked })}
            disabled={!canModify}
          />
          ...
          <SettingRow
            ...
            onCheckedChange={(checked) => onChange?.({ preventForwarding: checked })}
            disabled={!canModify}
          />
          ...
          <SettingRow
            ...
            onCheckedChange={(checked) => onChange?.({ preventCopying: checked })}
            disabled={!canModify}
          />
          ...
          <SettingRow
            ...
            onCheckedChange={(checked) => onChange?.({ hideNotificationContent: checked })}
            disabled={!canModify}
          />
        </div>
      </CardContent>
