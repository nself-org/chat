...
export class FacebookProvider extends BaseAuthProvider {
  ...
  async signIn(credentials: AuthCredentials): Promise<AuthResult> {
    if (!this.isEnabled()) {
      return this.createErrorResult(
        this.createError('PROVIDER_DISABLED', 'Facebook authentication is not enabled')
      )
    }
  ...
  async handleCallback(params: URLSearchParams): Promise<AuthResult> {
    ...
    if (code) {
      try {
        const response = await fetch(`${this.getAuthApiUrl()}/signin/provider/facebook/callback`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            code,
