      ...
      immer((set, get) => ({
          ...
          try {
            ...
            set(
              (draft) => {
                draft.isLoading = false;
                draft.error =
                  error instanceof Error ? error.message : 'Failed to initialize locale';
                draft.isInitialized = true;
              },
              false,
        ...
        setLocale: async (locale: LocaleCode) => {
          ...
          try {
            ...
            console.error('[i18n] Failed to set locale:', error);
            set(
              (draft) => {
                draft.isLoading = false;
                draft.error =
                  error instanceof Error ? error.message : 'Failed to set locale';
              },
              false,
        ...
        loadAllNamespaces: async (locale?: LocaleCode) => {
          ...
          try {
            ...
            console.error('[i18n] Failed to load all namespaces:', error);
            set(
              (draft) => {
                draft.isLoading = false;
              },
