...
export interface DMActions {
  ...
  updateNotificationPreference: (dmId: string, updates: Partial<DMNotificationPreference>) => void
  ...
  updateParticipant: (dmId: string, userId: string, updates: Partial<DMParticipant>) => void
            ...
            set(
              (state) => {
                if (state.activeDMId !== dmId) {
                  ...
                  if (dmId) {
                    state.recentDMs = [dmId, ...state.recentDMs.filter((id) => id !== dmId)].slice(
                      0,
                      MAX_RECENT_DMS
                    )
                  }
...
export const selectMessages = (dmId: string) => (state: DMStore) => state.messagesByDM[dmId] ?? []

export const selectTypingUsers = (dmId: string) => (state: DMStore) =>
  state.typingIndicators[dmId] ?? []

export const selectIsDMMuted = (dmId: string) => (state: DMStore) => state.mutedDMs.has(dmId)

export const selectIsDMStarred = (dmId: string) => (state: DMStore) => state.starredDMs.has(dmId)

export const selectPinnedMessages = (dmId: string) => (state: DMStore) =>
  state.pinnedMessages[dmId] ?? []

export const selectSharedFiles = (dmId: string) => (state: DMStore) => state.sharedFiles[dmId] ?? []

export const selectMediaItems = (dmId: string) => (state: DMStore) => state.mediaItems[dmId] ?? []
