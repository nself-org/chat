...
export class GitHubProvider extends BaseAuthProvider {
  ...
  async handleCallback(params: URLSearchParams): Promise<AuthResult> {
    ...
    if (error) {
      return this.createErrorResult(
        this.createError('OAUTH_ERROR', params.get('error_description') || 'GitHub authentication failed')
      )
    }
    ...
    if (refreshToken) {
      try {
        ...
        if (!response.ok) {
          return this.createErrorResult(
            this.createError('AUTH_FAILED', data.error?.message || 'Failed to authenticate with GitHub')
          )
        }
  ...
  private persistSession(session: { accessToken: string; refreshToken: string }): void {
    if (typeof window === 'undefined') return
    localStorage.setItem('nchat-github-session', JSON.stringify({
      ...session,
      timestamp: Date.now(),
    }))
  }
