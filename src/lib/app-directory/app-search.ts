...
function filterApps(apps: App[], params: AppSearchParams): App[] {
  return apps.filter((app) => {
    ...
    if (params.categories && params.categories.length > 0) {
      const hasCategory = app.categories.some((cat) => params.categories!.includes(cat.id))
 ...
 */
export function getRecommendedApps(installedAppIds: string[], limit: number = 10): App[] {
