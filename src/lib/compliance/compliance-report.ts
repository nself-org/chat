...
export function generateConsentStatusData(
  ...
  for (const consent of consents) {
    ...
    typeData.percentage = totalUsers > 0 ? Math.round((typeData.granted / totalUsers) * 100) : 0
  }

  // Calculate overall compliance rate
  const totalGranted = Object.values(consentsByType).reduce((sum, t) => sum + t.granted, 0)
  const totalRecords = consents.length
  const complianceRate = totalRecords > 0 ? Math.round((totalGranted / totalRecords) * 100) : 0
...
export function generateLegalHoldSummaryData(holds: LegalHold[]): LegalHoldSummaryData {
  ...
  return {
    ...
    holdsByStatus,
    totalCustodians,
    averageHoldDuration: completedHolds > 0 ? Math.round(totalDuration / completedHolds) : 0,
  }
}
...
export function calculateAuditStatistics(entries: ComplianceAuditEntry[]): {
  ...
  return {
    ...
    successRate: entries.length > 0 ? Math.round((successCount / entries.length) * 100) : 100,
    entriesByDay,
    topActions,
  }
}
