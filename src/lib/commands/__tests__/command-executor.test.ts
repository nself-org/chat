...
describe('executeCommand', () => {
  describe('validation', () => {
    it('should return error for invalid command', async () => {
      const parsed = createParsedCommand('test', {}, {
        valid: false,
  ...
  describe('fun commands', () => {
    it('should execute /shrug command', async () => {
      const parsed = createParsedCommand('shrug', {
        text: { name: 'text', rawValue: 'dunno', value: 'dunno', type: 'text', position: 0, valid: true },
      });
    ...
    it('should execute /me command', async () => {
      const parsed = createParsedCommand('me', {
        action: { name: 'action', rawValue: 'waves hello', value: 'waves hello', type: 'text', position: 0, valid: true },
      });
  ...
  describe('status commands', () => {
    it('should execute /status command with emoji and text', async () => {
      const parsed = createParsedCommand('status', {
        emoji: { name: 'emoji', rawValue: ':coffee:', value: ':coffee:', type: 'emoji', position: 0, valid: true },
        text: { name: 'text', rawValue: 'Having coffee', value: 'Having coffee', type: 'text', position: 1, valid: true },
      });
    ...
    it('should execute /dnd command with duration', async () => {
      const parsed = createParsedCommand('dnd', {
        duration: { name: 'duration', rawValue: '30m', value: '30 minutes', type: 'duration', position: 0, valid: true },
      });
  ...
  describe('channel commands', () => {
    ...
    it('should execute /topic command', async () => {
      const parsed = createParsedCommand('topic', {
        topic: { name: 'topic', rawValue: 'New topic', value: 'New topic', type: 'text', position: 0, valid: true },
      });
      const context = createContext({ channelId: 'channel-123' });
    ...
    it('should execute /rename command', async () => {
      const parsed = createParsedCommand('rename', {
        name: { name: 'name', rawValue: 'new-name', value: 'new-name', type: 'text', position: 0, valid: true },
      });
      const context = createContext({ channelId: 'channel-123' });
    ...
    it('should execute /invite command', async () => {
      const parsed = createParsedCommand('invite', {
        users: { name: 'users', rawValue: '@john', value: 'john', type: 'user', position: 0, valid: true },
      });
      const context = createContext({ channelId: 'channel-123' });
  ...
  describe('moderation commands', () => {
    it('should execute /kick command with preview', async () => {
      const parsed = createParsedCommand('kick', {
        user: { name: 'user', rawValue: '@john', value: 'john', type: 'user', position: 0, valid: true },
      });
      const context = createContext({ channelId: 'channel-123', channelName: 'general' });
    ...
    it('should execute /ban command with preview', async () => {
      const parsed = createParsedCommand('ban', {
        user: { name: 'user', rawValue: '@john', value: 'john', type: 'user', position: 0, valid: true },
        reason: { name: 'reason', rawValue: 'Spamming', value: 'Spamming', type: 'text', position: 1, valid: true },
      });
      const context = createContext({ channelId: 'channel-123' });
    ...
    it('should execute /slowmode command', async () => {
      const parsed = createParsedCommand('slowmode', {
        interval: { name: 'interval', rawValue: '30s', value: '30 seconds', type: 'duration', position: 0, valid: true },
      });
      const context = createContext({ channelId: 'channel-123' });
  ...
  describe('utility commands', () => {
    it('should execute /remind command', async () => {
      const parsed = createParsedCommand('remind', {
        time: { name: 'time', rawValue: '30m', value: '30 minutes', type: 'duration', position: 0, valid: true },
        message: { name: 'message', rawValue: 'Check emails', value: 'Check emails', type: 'text', position: 1, valid: true },
      });
    ...
    it('should fail /remind without time', async () => {
      const parsed = createParsedCommand('remind', {
        message: { name: 'message', rawValue: 'Test', value: 'Test', type: 'text', position: 1, valid: true },
      });
    ...
    it('should execute /search command', async () => {
      const parsed = createParsedCommand('search', {
        query: { name: 'query', rawValue: 'test search', value: 'test search', type: 'text', position: 0, valid: true },
      });
  ...
  describe('navigation commands', () => {
    it('should execute /open command', async () => {
      const parsed = createParsedCommand('open', {
        channel: { name: 'channel', rawValue: '#general', value: 'general', type: 'channel', position: 0, valid: true },
      });
  ...
  describe('media commands', () => {
    it('should execute /giphy command with preview', async () => {
      const parsed = createParsedCommand('giphy', {
        query: { name: 'query', rawValue: 'cats', value: 'cats', type: 'text', position: 0, valid: true },
      });
    ...
    it('should execute /poll command with preview', async () => {
      const parsed = createParsedCommand('poll', {
        question: { name: 'question', rawValue: 'Favorite color?', value: 'Favorite color?', type: 'text', position: 0, valid: true },
        options: { name: 'options', rawValue: '"Red" "Blue"', value: ['Red', 'Blue'], type: 'options', position: 1, valid: true },
      });
    ...
    it('should fail /poll without enough options', async () => {
      const parsed = createParsedCommand('poll', {
        question: { name: 'question', rawValue: 'Question?', value: 'Question?', type: 'text', position: 0, valid: true },
        options: { name: 'options', rawValue: '"Only one"', value: ['Only one'], type: 'options', position: 1, valid: true },
      });
