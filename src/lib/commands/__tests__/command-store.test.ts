...
describe('Selectors', () => {
  ...
  it('selectRecentCommandNames should return unique command names', () => {
    ...
    act(() => {
      result.current.addToHistory({ commandString: '/shrug', commandName: 'shrug', executedAt: Date.now(), success: true });
      result.current.addToHistory({ commandString: '/shrug', commandName: 'shrug', executedAt: Date.now(), success: true });
      result.current.addToHistory({ commandString: '/status', commandName: 'status', executedAt: Date.now(), success: true });
      result.current.addToHistory({ commandString: '/failed', commandName: 'failed', executedAt: Date.now(), success: false });
    });
