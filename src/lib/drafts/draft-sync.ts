...
export class DraftSyncManager {
   ...
   */
  canSync(): boolean {
    return this.config.enabled && this.apiClient !== null && this.userId !== null
  }
  ...
  async sync(): Promise<DraftSyncResult> {
    ...
    try {
      ...
      const remoteResponse = await this.apiClient!.getDrafts(this.userId!, this.lastSyncTimestamp)
 ...
 */
export function getSyncManager(config?: Partial<DraftSyncConfig>): DraftSyncManager {
...
export function createMockSyncApiClient(): SyncApiClient {
  ...
  return {
    async getDrafts(userId: string, since?: number): Promise<SyncResponse> {
      ...
      const drafts = Array.from(userDrafts.values()).filter((d) => !since || d.lastModified > since)
