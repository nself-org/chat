...
export function formatAsMessageContent(embed: SocialEmbed): any {
  ...
  if (embed.engagement) {
    const stats: string[] = []
    if (embed.engagement.likes) stats.push(`❤️ ${formatNumber(embed.engagement.likes)}`)
    if (embed.engagement.retweets) stats.push(`🔄 ${formatNumber(embed.engagement.retweets)}`)
    if (embed.engagement.shares) stats.push(`🔗 ${formatNumber(embed.engagement.shares)}`)
    if (embed.engagement.comments) stats.push(`💬 ${formatNumber(embed.engagement.comments)}`)
    if (embed.engagement.replies) stats.push(`💬 ${formatNumber(embed.engagement.replies)}`)
    if (stats.length > 0) {
      engagementText = `\n\n${stats.join(' • ')}`
    }
  }
