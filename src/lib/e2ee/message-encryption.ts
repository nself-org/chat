...
export async function encryptMessageForSending(
  ...
  try {
    ...
    const deviceId = options.recipientDeviceId || await getDefaultDeviceId(
      options.recipientUserId,
      apolloClient
    );

    if (!deviceId) {
      console.warn('No device ID found for recipient, sending unencrypted');
      return {
        isEncrypted: false,
        plainContent: plaintext,
      };
...
async function getDefaultDeviceId(
  ...
  try {
    ...
    console.error('Error getting default device ID:', error);
    return null;
  }
}
...
export async function encryptMessagesForGroup(
  ...
  // TODO: Implement sender key distribution for groups
