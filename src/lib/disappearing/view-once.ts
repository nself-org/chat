 ...
 */
export function isViewOnceMessage(disappearing?: DisappearingMessageData | null): boolean {
...
export class ViewOnceSession {
  ...
  constructor(messageId: string, channelId: string, viewerId: string, onComplete?: () => void) {
 ...
 */
export function getBlurredPlaceholderStyles(thumbnailUrl?: string): React.CSSProperties {
  if (!thumbnailUrl) {
    return {
 ...
 */
export function getViewOnceStatusText(state: ViewOnceState, isOwnMessage: boolean): string {
