...
import { aggregateActivities, isAggregatedActivity, flattenAggregatedActivities } from './activity-aggregator';
...
export function groupActivitiesByDate(
  ...
  return Array.from(groups.values()).sort((a, b) => {
    const dateA = new Date(a.activities[0] ? (isAggregatedActivity(a.activities[0]) ? a.activities[0].latestAt : a.activities[0].createdAt) : '');
    const dateB = new Date(b.activities[0] ? (isAggregatedActivity(b.activities[0]) ? b.activities[0].latestAt : b.activities[0].createdAt) : '');
