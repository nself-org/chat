...
export class EmailSender {
  ...
  async send(email: Email): Promise<EmailSendResult> {
    ...
    try {
      ...
      console.log(`Email sent via ${this.config.provider} in ${Date.now() - startTime}ms`);
      return result;
    } catch (error) {
    ...
    while (this.emailQueue.length > 0) {
      ...
      if (result.success) {
        ...
        if (email.attempts >= email.maxAttempts) {
          ...
          this.emailQueue.shift();
          console.error(`Email ${email.id} failed after ${email.attempts} attempts:`, result.error);
        } else {
          // Retry with exponential backoff
          const delay = this.retryIntervals[Math.min(email.attempts - 1, this.retryIntervals.length - 1)];
          console.log(`Retrying email ${email.id} in ${delay}ms (attempt ${email.attempts + 1}/${email.maxAttempts})`);
