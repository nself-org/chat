...
const MODIFIER_KEYS = new Set(['mod', 'ctrl', 'alt', 'shift', 'meta', 'control', 'option', 'command']);
 ...
 */
export function splitShortcutForDisplay(shortcut: string, useMacSymbols: boolean = isMacOS()): string[] {
  const parsed = parseShortcut(shortcut);
  const keyMap = useMacSymbols ? MAC_KEY_SYMBOLS : WINDOWS_KEY_NAMES;
  const parts: string[] = [];
