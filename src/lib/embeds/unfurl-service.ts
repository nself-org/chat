...
export async function fetchOEmbed(html: string, url: string): Promise<OEmbedData | null> {
  ...
  if (!oEmbedMatch) {
    // Try reversed format
    const oEmbedMatchReversed = html.match(/<link[^>]+href=["']([^"']+)["'][^>]+type=["']application\/json\+oembed["']/i);
...
export async function parseHtmlForUnfurl(
  ...
  const data: UnfurlData = {
    ...
    imageHeight:
      openGraph.imageHeight || oEmbed?.thumbnailHeight || twitterCard.playerHeight,
    imageAlt: twitterCard.imageAlt || openGraph.imageAlt,
    // Site info
    favicon: resolveUrl(url, basicMeta.favicon || ''),
    siteName:
      oEmbed?.providerName || openGraph.siteName,
    // Author info
    ...
    video: resolveUrl(
      url,
      openGraph.video || twitterCard.player || ''
    ),
