...
export function createWelcomeWorkflow(
  ...
  const welcomeMessage = createMessageStep({ x: 250, y: 180 }, {
    target: 'user',
...
Would you like to introduce yourself to the team?`,
  ...
  const introForm = createFormStep({ x: 250, y: 310 }, {
  ...
  checkPost.config = {
    logic: 'and',
    conditions: [
      createCondition('variables.formResponse.postIntroduction', 'equals', true),
    ],
  }

  // Create introduction post message
  const postIntro = createMessageStep({ x: 100, y: 570 }, {
    target: 'channel',
...
Everyone say hi!`,
  ...
  const thankYouPost = createMessageStep({ x: 100, y: 700 }, {
    target: 'user',
    content: 'Thanks for introducing yourself! Your introduction has been posted to the team. Looking forward to working with you!',
  ...
  const thankYouNoPost = createMessageStep({ x: 400, y: 570 }, {
    target: 'user',
    content: 'No problem! Your information has been saved. Feel free to introduce yourself to the team whenever you\'re ready. Welcome aboard!',
    parseVariables: false,
  })
  ...
  // Create end steps
  const endPosted = createEndStep({ x: 100, y: 830 }, {
    status: 'success',
    message: 'Welcome workflow completed - introduction posted',
  })
  endPosted.name = 'End (Posted)'

  const endNotPosted = createEndStep({ x: 400, y: 700 }, {
    status: 'success',
