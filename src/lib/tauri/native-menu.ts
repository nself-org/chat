...
export type NavigationTarget = 'home' | 'channels' | 'messages' | 'threads' | 'settings'
 ...
 */
export async function setupMenuListeners(handlers: MenuEventHandlers): Promise<() => void> {
  if (!isTauri()) {
    return () => {}
  }
  ...
  if (handlers.onKeyboardShortcuts) {
    const unsub = await listen('menu-keyboard-shortcuts', handlers.onKeyboardShortcuts)
    unsubscribers.push(unsub)
  }
