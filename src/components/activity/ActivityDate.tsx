...
export function ActivityDate({ date, className, format = 'smart' }: ActivityDateProps) {
    ...
    if (format === 'relative' || format === 'smart') {
      const interval = setInterval(() => {
        ...
        if (diffMs < 60 * 1000) {
          ...
          setFormattedDate(
            format === 'smart' ? formatRelativeTimeShort(date) : formatRelativeTime(date)
          )
        }
 ...
 */
export function ActivityDateSeparator({ label, className }: { label: string; className?: string }) {
  return (
