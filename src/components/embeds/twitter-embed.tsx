  ...
  React.useEffect(() => {
    ...
    function renderTweet() {
      ...
      if (twttr && twttr.widgets && embedRef.current) {
        ...
        twttr.widgets.createTweet(tweetId!, embedRef.current, {
  ...
  if (useNativeEmbed) {
    return (
      <div className={cn('relative max-w-lg', className)}>
        ...
        <div
          ref={embedRef}
          className="min-h-[100px] [&_.twitter-tweet]:!m-0"
        >
          {/* Loading placeholder */}
  ...
  return (
    <div
      className={cn(
        ...
        'hover:border-[#1DA1F2]/30 cursor-pointer transition-colors',
        'max-w-lg',
        className
      )}
      onClick={handleClick}
    ...
    >
      ...
      <div className="flex items-start gap-3">
        {avatarUrl ? (
          ...
          <div className="h-12 w-12 rounded-full bg-[#1DA1F2] flex items-center justify-center">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            ...
            {verified && <VerifiedIcon className="h-4 w-4 text-[#1DA1F2] flex-shrink-0" />}
          </div>
          <span className="text-sm text-muted-foreground">@{username}</span>
        </div>

        <TwitterIcon className="h-5 w-5 text-[#1DA1F2] flex-shrink-0" />
      </div>

      {bio && (
        <p className="mt-3 text-sm text-foreground line-clamp-2">{bio}</p>
      )}
