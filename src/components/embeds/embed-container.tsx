...
export function EmbedContainer({
  ...
  if (loading && shouldUnfurl) {
    return <EmbedSkeleton embedType={embedType} compact={compact} className={className} />
  }
...
function CollapsedEmbed({ url, embedType, onExpand, onClose, className }: CollapsedEmbedProps) {
  const getIcon = () => {
    switch (embedType) {
      case 'twitter':
  ...
  return (
    ...
    >
      {getIcon()}
      <span className="flex-1 truncate text-sm text-muted-foreground">{getDomain()}</span>
...
function EmbedError({ url, error, showCloseButton, onClose, className }: EmbedErrorProps) {
  const handleClick = () => {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-foreground">{getDomain()}</p>
...
function GenericLinkEmbed({
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 truncate text-sm text-foreground">{getDomain()}</span>
