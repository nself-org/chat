...
export function LanguageSwitcher({
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {showNativeName && config.name !== config.englishName && (
              <span className="text-xs text-muted-foreground">({config.englishName})</span>
            )}
          </div>
  ...
  const defaultTrigger = compact ? (
    ...
    <Button variant="outline" className={cn('gap-2', className)} disabled={isLoading}>
      ...
      <span>{showNativeName ? currentLocaleConfig?.name : currentLocaleConfig?.englishName}</span>
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DropdownMenuContent align={align} className="w-56">
        ...
        <DropdownMenuSeparator />
        <div className="max-h-[300px] overflow-y-auto">{locales.map(renderLocaleItem)}</div>
      </DropdownMenuContent>
    </DropdownMenu>
  )
}
