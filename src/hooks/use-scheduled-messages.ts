...
export function useScheduledMessages(channelId?: string) {
  ...
  const scheduleMessage = useCallback(async (options: CreateScheduledMessageOptions) => {
    if (!user) {
      toast({
        title: 'Error',
  ...
  const updateMessage = useCallback(async (messageId: string, options: UpdateScheduledMessageOptions) => {
    try {
      ...
      logger.error('Failed to update scheduled message', error instanceof Error ? error : new Error(String(error)))
      toast({
        title: 'Error',
  ...
  const cancelMessage = useCallback((messageId: string) => {
    try {
      ...
      logger.error('Failed to cancel scheduled message', error instanceof Error ? error : new Error(String(error)))
      toast({
        title: 'Error',
  ...
  const deleteMessage = useCallback((messageId: string) => {
    try {
      ...
      logger.error('Failed to delete scheduled message', error instanceof Error ? error : new Error(String(error)))
      toast({
        title: 'Error',
