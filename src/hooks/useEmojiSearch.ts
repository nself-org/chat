...
export function useEmojiSearch(options: UseEmojiSearchOptions = {}): UseEmojiSearchReturn {
  const {
    limit = 50,
    includeCustom = true,
    category = 'all',
...
export function useQuickSearch(limit: number = 8): UseQuickSearchReturn {
  const [results, setResults] = useState<Array<{ emoji: string; shortcode: string; name: string }>>(
    []
  )
