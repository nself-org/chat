...
export function useMessageTranslation(options: UseMessageTranslationOptions = {}) {
  ...
  const translateMessage = useCallback(async (
    ...
    try {
      ...
      logger.error('Failed to translate message', error instanceof Error ? error : undefined, { messageId, errorMessage: errorMsg })
  ...
  const retryTranslation = useCallback((messageId: string, text: string, targetLanguage?: string) => {
    setErrors(prev => {
      const next = new Map(prev)
      next.delete(messageId)
      return next
