...
import { getOfflineQueue, type QueueEventListener } from '@/lib/offline/offline-queue'
import type {
  QueuedAction,
  QueuedSendMessage,
  QueuedEditMessage,
...
export function usePendingMessageCount(channelId: string): number {
  return useOfflineStore(
    (state) =>
      state.queuedActions.filter((a) => a.channelId === channelId && a.type === 'send_message')
        .length + state.pendingMessages.filter((m) => m.channelId === channelId).length
  )
}
...
export function useHasPendingChanges(): boolean {
  return useOfflineStore(
    (state) => state.queuedActions.length > 0 || state.pendingMessages.length > 0
  )
}
