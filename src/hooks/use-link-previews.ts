...
export function useLinkPreviews(text?: string, options: UseLinkPreviewsOptions = {}) {
  ...
  const fetchPreview = useCallback(async (url: string) => {
    try {
      ...
      if (result.success && result.preview) {
        ...
        logger.warn('Failed to fetch link preview', { url, error: errorMsg, errorCode: result.errorCode })
        return null
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : 'Unknown error'
      setErrors(prev => new Map(prev).set(url, errorMsg))
      logger.error('Error fetching link preview', error instanceof Error ? error : undefined, { url, errorMessage: errorMsg })
      return null
    } finally {
      setLoading(prev => {
        const next = new Map(prev)
  ...
  const refetchPreview = useCallback((url: string) => {
