...
export function useOfflineCache(): UseOfflineCacheReturn {
  ...
  const refreshStats = useCallback(async () => {
    try {
      ...
      console.error('[useOfflineCache] Failed to refresh stats:', error);
    }
  }, [store]);
  ...
  const getMessages = useCallback(
    async (channelId: string, limit?: number) => {
      const cache = getOfflineCache();
  ...
  useEffect(() => {
    ...
    async function loadMessages() {
      ...
      try {
        ...
        console.error('[useCachedMessages] Error:', error);
      } finally {
        if (mounted) {
          setIsLoading(false);
        }
...
export function useIsChannelCached(channelId: string): boolean {
  return useOfflineStore((state) =>
    state.cachedChannelIds.includes(channelId)
  );
}
...
export function useOfflineSync() {
  ...
  const sync = useCallback(async (options?: { fullSync?: boolean }) => {
    try {
