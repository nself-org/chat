  ...
  return (
    <div className="performance-dashboard">
      ...
      <div className="custom-metrics">
        ...
        <div>
          Render Time: {stats.renderTime.avg.toFixed(2)}ms
        </div>
        <div>
          Memory Usage: {stats.memoryUsage.avg.toFixed(1)}%
        </div>
      </div>
...
function enforcePerformanceBudget() {
  ...
  if (snapshot.webVitals.lcp && snapshot.webVitals.lcp > PERFORMANCE_BUDGET.lcp) {
    violations.push(`LCP exceeded budget: ${snapshot.webVitals.lcp}ms > ${PERFORMANCE_BUDGET.lcp}ms`);
  }

  if (snapshot.webVitals.fid && snapshot.webVitals.fid > PERFORMANCE_BUDGET.fid) {
    violations.push(`FID exceeded budget: ${snapshot.webVitals.fid}ms > ${PERFORMANCE_BUDGET.fid}ms`);
  }
  ...
  if (snapshot.custom.apiResponseTime > PERFORMANCE_BUDGET.apiResponseTime) {
    violations.push(`API response time exceeded budget: ${snapshot.custom.apiResponseTime.toFixed(0)}ms > ${PERFORMANCE_BUDGET.apiResponseTime}ms`);
  }

  if (snapshot.custom.renderTime > PERFORMANCE_BUDGET.renderTime) {
    violations.push(`Render time exceeded budget: ${snapshot.custom.renderTime.toFixed(2)}ms > ${PERFORMANCE_BUDGET.renderTime}ms`);
  }

  if (snapshot.custom.memoryUsage > PERFORMANCE_BUDGET.memoryUsage) {
    violations.push(`Memory usage exceeded budget: ${snapshot.custom.memoryUsage.toFixed(1)}% > ${PERFORMANCE_BUDGET.memoryUsage}%`);
  }
