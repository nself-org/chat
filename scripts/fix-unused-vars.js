...
for (const line of lines) {
  ...
  const match = line.match(/^\d+:\d+\s+Warning: '(.+?)' is (?:defined but never used|assigned a value but never used)\. Allowed unused vars must match/);
...
for (const [filepath, varNames] of fixes.entries()) {
  try {
    ...
    if (modified) {
      ...
      filesModified++;
      console.log(`✓ Fixed ${varNames.size} variable(s) in ${path.relative(process.cwd(), filepath)}`);
    }
  } catch (error) {
